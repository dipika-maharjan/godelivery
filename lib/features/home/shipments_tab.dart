        ...
        children: [
          ...
          Expanded(
            child: RefreshIndicator(
              onRefresh: () =>
                  ref.read(ordersProvider(OrderRoleFilter.sent).notifier).refresh(),
              child: orders.when(
                loading: () => const Center(child: CircularProgressIndicator()),
                error: (error, _) => _ErrorState(
                  message: error is ApiException ? error.message : 'Something went wrong.',
                  onRetry: () => ref.invalidate(ordersProvider(OrderRoleFilter.sent)),
                ),
                data: (list) => list.isEmpty
                    ? const _EmptyState(
                        icon: LucideIcons.send,
                        message: "You haven't sent any shipments yet. Tap + to create one.",
                      )
                    : ListView.separated(
